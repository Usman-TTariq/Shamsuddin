...
jQuery(document).ready( function($){
	...
	$(document).on('click', '.em-view-container .em-ajax.em-pagination a.page-numbers', function(e){
		...
		let url_params = new URL(href%2c%20window.location.html).searchParams;
	...
	const em_calendar_init = function( calendar ){
		...
		calendar.on('click', 'a.em-calnav, a.em-calnav-today', function(e){
			...
			if( custom_data.length > 0 ){
				...
				let $URL = new URL(url%2c%20window.location.html);
	...
	document.querySelectorAll('form.em-ajax-form').forEach( function(el){
		el.addEventListener('submit', function(e){
			...
			if( form.getAttribute('data-api-url') ){
				request.open('POST.html', form.getAttribute('data-api-url'), true);
				request.setRequestHeader('X-WP-Nonce', EM.api_nonce);
			}else{
				request.open('POST.html', EM.ajaxurl, true);
			}
